from uagents import Context,Agent,Bureau,Model
...
alice = Agent(name='alice',seed='Alice seed')
bob = Agent(name='bob',seed='bob seed')
...
async def request_funds(ctx : Context):
    await ctx.send(
        bob.address,
        PaymentRequest(
            wallet_add=str(alice.wallet.address()),amount = AMOUNT,denom = DENOM    
        )
    )

@alice.on_message(model=TransactionInfo)
async def confirm_Transaction(ctx : Context,sender :str,msg : TransactionInfo):
    ctx.logger.info(f'Recieved Transaction info from {sender} : {msg} ')
...
async def send_payment(ctx:Context,sender :str,msg:PaymentRequest):
    ctx.logger.info(f'Recieved Transaction Request from {sender} : {msg}')

    transaction = ctx.ledger.send_tokens(
        msg.wallet_add,msg.amount,msg.denom,bob.wallet
    )
